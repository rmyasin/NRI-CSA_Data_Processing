...
% The '3sides' version of the code is edited for a marker that doesn't have
% all 4 sides
...
% A: 50
% B: 51
% C: 52 (not used for the 3-sided probe)
% D: 53
% Base Marker: 60

function tip_calibration = pivot_calibration_micron_3sides(filefolder, fileName)
labelList=[50,51,53]; % List order A, B, D

[~,~,micron,~,vProtocol]=readRobTxt(filefolder,fileName);

% for ii=1:length(micron)
%     micron(ii).pos=micron(ii).pos/1000;
% end
...
for ii=1:length(labelList)
    ...
    % Form matrices of rotations/positions
    ...
    if ii==2 % skip the bc transition, marker doesn't exist
        HNextMarker(:,:,ii)=eye(4);
    else
...
H_TipFrame(:,:,3)=eye(4);
H_TipFrame(:,:,4)=transformation(HNextMarker(1:3,1:3,3),tipLoc(:,3));
